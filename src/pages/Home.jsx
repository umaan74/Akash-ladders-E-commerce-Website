...
const Home = () => {
  ...
  return (
    <div className="space-y-16 md:space-y-20 pb-16">
      ...
      {/* Hero Section */}
      <section className="relative bg-slate-100 dark:bg-slate-950 overflow-hidden pt-10 md:pt-12 pb-16 md:pb-20 border-b border-slate-200 dark:border-slate-800 transition-colors">
        
        {/* Decorative Grid Lines */}
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#cbd5e1_1px,transparent_1px),linear-gradient(to_bottom,#cbd5e1_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#1e293b_1px,transparent_1px),linear-gradient(to_bottom,#1e293b_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] opacity-30"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
            ...
            <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
              ...
              <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-3.5 py-1.5 rounded-full text-xs font-bold text-amber-600 dark:text-amber-400">
              ...
              <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
                ...
                <span className="bg-gradient-to-r from-amber-500 via-amber-600 to-orange-500 dark:from-amber-400 dark:via-amber-500 dark:to-orange-500 bg-clip-text text-transparent">
                  Strength & Safety
                </span>
              </h1>
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-slate-800/80 grid grid-cols-3 gap-4 text-left">
                <div>
                  <div className="text-lg sm:text-2xl font-black text-slate-900 dark:text-white">EN131</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">Safety Certified</div>
                </div>
                <div>
                  <div className="text-lg sm:text-2xl font-black text-slate-900 dark:text-white">350 KG</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">Tested Load Margin</div>
                </div>
                <div>
                  <div className="text-lg sm:text-2xl font-black text-amber-600 dark:text-amber-400">5-10 YRS</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">Factory Warranty</div>
                </div>
              </div>
            ...
            <div className="lg:col-span-5 relative">
              <div className="relative rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl shadow-amber-500/10 group">
                <img
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border border-slate-200 dark:border-slate-700/80 p-4 rounded-2xl flex items-center justify-between shadow-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-slate-900 dark:text-white font-bold text-sm">Akash Titan Series</h4>
                      <p className="text-slate-500 dark:text-slate-400 text-xs">Aircraft Alloy 6063-T6</p>
                    </div>
                  </div>
                  <Link 
                    to="/products"
                    className="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-500/20 hover:bg-amber-500 hover:text-slate-950 transition-all"
                  >
                    View Models
                  </Link>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {safetyCertifications.map((item, index) => {
            ...
            return (
              <div 
                key={index}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl flex items-start gap-4 hover:border-amber-500/40 transition-colors shadow-sm"
              >
                ...
                <div>
                  ...
                  <p className="text-slate-600 dark:text-slate-400 text-xs mt-1 leading-relaxed">{item.description}</p>
                </div>
              </div>
            );
          })}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mt-1">Product Categories</h2>
            <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Ladders engineered for household, commercial, electrical, and industrial tasks.</p>
          </div>
          <Link 
            to="/products" 
            className="inline-flex items-center gap-1.5 text-amber-600 dark:text-amber-400 font-bold text-sm hover:underline"
          >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mt-1">Featured Ladders</h2>
            <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Best-selling safety ladders trusted by professionals across India.</p>
          </div>
          <Link 
            to="/products" 
            className="inline-flex items-center gap-1.5 text-amber-600 dark:text-amber-400 font-bold text-sm hover:underline"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              ...
              <div className="space-y-4 pt-2">
                {[
                  ...
                  <div key={idx} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                      ✓
                    </div>
                    <div>
                      <h4 className="text-slate-900 dark:text-white font-bold text-sm">{item.title}</h4>
                      <p className="text-slate-600 dark:text-slate-400 text-xs">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="lg:col-span-6 grid grid-cols-2 gap-4">
              <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-2 shadow-sm">
                <div className="text-4xl font-black text-amber-500">28+</div>
                <div className="text-slate-900 dark:text-white font-bold text-sm">Years Experience</div>
                <div className="text-slate-500 dark:text-slate-400 text-xs">Ladder Manufacturing</div>
              </div>
              <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-2 shadow-sm">
                <div className="text-4xl font-black text-slate-900 dark:text-white">500k+</div>
                <div className="text-slate-900 dark:text-white font-bold text-sm">Ladders Sold</div>
                <div className="text-slate-500 dark:text-slate-400 text-xs">Across Pan-India</div>
              </div>
              <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-2 shadow-sm">
                <div className="text-4xl font-black text-slate-900 dark:text-white">100%</div>
                <div className="text-slate-900 dark:text-white font-bold text-sm">EN131 Compliant</div>
                <div className="text-slate-500 dark:text-slate-400 text-xs">Safety Inspected</div>
              </div>
              <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-2 shadow-sm">
                <div className="text-4xl font-black text-amber-500">350kg</div>
                <div className="text-slate-900 dark:text-white font-bold text-sm">Max Load Margin</div>
                <div className="text-slate-500 dark:text-slate-400 text-xs">Heavy Industrial Rating</div>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mt-1">What Our Customers Say</h2>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">From site engineers to home owners, discover why customers rely on Akash Ladders.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t) => (
            <div key={t.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl flex flex-col justify-between space-y-4 shadow-sm">
              <div className="space-y-3">
                ...
                <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed italic">"{t.content}"</p>
              </div>
