...
import { Scale, Trash2, ShoppingBag, ArrowRight, Check, X } from 'lucide-react';
  ...
  if (compareItems.length === 0) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center space-y-6 text-slate-900 dark:text-white">
        ...
        <h2 className="text-3xl font-black">No Ladders Selected for Comparison</h2>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 space-y-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-6 gap-4">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Compare Ladders ({compareItems.length} Selected)</h1>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-x-auto shadow-sm dark:shadow-2xl">
          ...
          <tbody>
            {specRows.map((row, idx) => (
              <tr key={idx} className="border-b border-slate-200 dark:border-slate-800/60 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-950/40">
                <td className="p-4 font-bold text-slate-500 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-950/40">
                  {row.label}
                </td>
                {compareItems.map((p) => (
                  <td key={p.id} className="p-4 border-l border-slate-200 dark:border-slate-800/80">
                    {row.render(p)}
                  </td>
                ))}
              </tr>
