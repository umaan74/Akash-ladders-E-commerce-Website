...
const CustomerDashboard = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'Pending':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20"><Clock className="w-3.5 h-3.5" /> Order Placed</span>;
      case 'Processing':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-blue-500/10 text-blue-400 border border-blue-500/20"><Package className="w-3.5 h-3.5" /> Preparing Dispatch</span>;
      case 'Shipped':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-purple-500/10 text-purple-400 border border-purple-500/20"><Truck className="w-3.5 h-3.5" /> In Freight Transit</span>;
      case 'Delivered':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><CheckCircle2 className="w-3.5 h-3.5" /> Delivered</span>;
      case 'Cancelled':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-red-500/10 text-red-400 border border-red-500/20"><AlertCircle className="w-3.5 h-3.5" /> Cancelled</span>;
      default:
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-slate-800 text-slate-300">{status}</span>;
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Header Banner */}
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 z-10">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs sm:text-sm mt-0.5">Manage your ladder orders and track freight dispatches</p>
          </div>
        </div>

        {/* Dashboard Nav Tabs */}
        <div className="flex items-center gap-2 z-10 w-full md:w-auto border-t md:border-t-0 border-slate-800 pt-4 md:pt-0">
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-medium">Total Orders Placed</p>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-medium">Active Dispatches</p>
            <p className="text-2xl font-black text-amber-400 mt-1">{activeOrdersCount}</p>
          </div>
          <div className="w-12 h-12 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-400">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-medium">Total Procurement Spend</p>
            <p className="text-2xl font-black text-white mt-1">₹{totalSpent.toLocaleString('en-IN')}</p>
          </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                    <div>
                      <div className="flex items-center gap-3">
                        <span className="text-base font-black text-amber-400">{order.orderId}</span>
                    ...
                    <div className="flex items-center gap-4 justify-between sm:justify-end">
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 bg-slate-950 hover:bg-slate-800 text-amber-400 border border-slate-700 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors"
                      >
                  ...
                  <div className="flex items-center gap-3 overflow-x-auto py-1">
                    {order.items?.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800 text-xs shrink-0">
                        {item.image && (
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-white font-bold text-base mt-0.5">{user?.phone || 'Not provided'}</p>
              </div>
            </div>

            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-1">
              <p className="text-xs text-slate-400">Saved Shipping Address</p>
              <p className="text-white font-medium">{user?.address || 'Not provided'}</p>
              {user?.city && <p className="text-slate-400 text-xs">{user.city} {user.pincode ? `- ${user.pincode}` : ''}</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full my-8 overflow-hidden shadow-2xl space-y-6 p-6">
            
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div>
                <span className="text-xs uppercase tracking-widest font-bold text-amber-400">Dispatch Order Tracking</span>
                <h3 className="text-xl font-black text-white mt-1">Order {selectedOrder.orderId}</h3>
              </div>
              <button
            ...
            <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-3">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Freight Progress</p>
                  ...
                  return (
                    <div key={st} className="space-y-1.5">
                      <div className={`h-2 rounded-full transition-all ${
                        isPassed ? 'bg-amber-500' : 'bg-slate-800'
                      }`} />
              ...
              <div className="bg-slate-950 rounded-2xl border border-slate-800 divide-y divide-slate-900">
                {selectedOrder.items?.map((item, idx) => (
                  <div key={idx} className="p-3.5 flex items-center justify-between text-xs">
                    ...
                    <span className="text-amber-400 font-extrabold">₹{(item.price * item.quantity).toLocaleString('en-IN')}</span>
                  </div>
                ))}
              </div>
            </div>
