...
import { X, CheckCircle2, Truck, ShieldCheck, CreditCard, Building2, User, Phone, Mail, MapPin } from 'lucide-react';
...
const CheckoutModal = ({ onClose }) => {
  ...
  if (orderPlaced) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 sm:p-8 text-center space-y-6 animate-in zoom-in-95 duration-200">
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-white mt-3">Thank You for Your Order!</h2>
            <p className="text-slate-400 text-sm mt-1">
              Your order ID is <strong className="text-amber-400">{orderId}</strong>
            </p>
          </div>
          ...
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 text-left text-xs space-y-2 text-slate-300">
            ...
            <div className="flex justify-between border-b border-slate-800 pb-2">
              ...
              <span className="font-semibold text-white truncate max-w-[200px]">{formData.address}, {formData.city}</span>
            </div>
          ...
          <p className="text-xs text-slate-400">
            Our dispatch coordinator <strong className="text-white">Akash Ladders Sales Team (+91 8898133393)</strong> will contact you shortly to confirm freight dispatch details.
          </p>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-3xl w-full my-8 overflow-hidden shadow-2xl">
        
        {/* Header */}
        <div className="p-5 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-6 h-6 text-amber-500" />
            <h2 className="text-lg font-bold text-white">Akash Ladders — Direct Dispatch Checkout</h2>
          </div>
        ...
        <form onSubmit={handleSubmitOrder} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-9 pr-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500 resize-none"
                />
              </div>
            </div>
          ...
          <div className="space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2 text-xs text-slate-300 max-h-40 overflow-y-auto">
                {cart.map(item => (
                  <div key={item.product.id} className="flex justify-between items-center py-1 border-b border-slate-900 last:border-0">
                    <span className="truncate max-w-[180px] text-white font-medium">{item.product.name}</span>
                    <span className="text-slate-400">x{item.quantity}</span>
                    <span className="text-amber-400 font-bold">₹{(item.product.price * item.quantity).toLocaleString('en-IN')}</span>
                  </div>
                ))}
              </div>
